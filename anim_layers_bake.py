...
def smart_start_end(keyframes, frame_start, frame_end):
    ...
    if min(frames) < frame_start:
        keyframes.append((frame_start, 'LINEAR', 'VECTOR', 'VECTOR',keyframes[0][4], keyframes[0][5]))
    if max(frames) > frame_end:
        keyframes.append((frame_end, 'LINEAR', 'VECTOR', 'VECTOR', keyframes[-1][4], keyframes[-1][5]))
    keyframes.sort()
    return keyframes
...
def AL_bake(frame_start, frame_end, nla_tracks, fcu_keys):
    ...
    for fcu_key, values in fcu_keys.items():
        if obj.als.onlyselected:
            ...
            if fcu_key[0].split('].')[0]+']' not in bones and fcu_key[0] not in transform_types:
                continue
        ...
        for frame in range(frame_start, frame_end+1):
            ...
            for track in nla_tracks[:-1]:
                ...
                if (fcu is None or fcu.mute) and track.strips[0].blend_type == 'REPLACE' and not evaluate:
                    if 'scale' in fcu_key[0] or ('rotation_quaternion' in fcu_key[0] and fcu_key[1] == 0):
                        if obj.type == 'ARMATURE':
...
class MergeAnimLayerDown(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if obj.als.direction == 'UP' and nla_tracks[obj.track_list_index].strips[0].blend_type == 'ADD' and obj.als.baketype == 'AL':
            additive = True
            blend = 'ADD'
        else:
            additive = False
        ...
        if obj.als.operator == 'MERGE':
            ...
            baked_layer = anim_layers.add_animlayer(layer_name = 'Baked_Layer' , duplicate = False, index = obj.track_list_index, blend_type = blend)
