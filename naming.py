...
D = bpy.data
number_tokens = []
side_pairs = [('l','r'), ('L', 'R'), ('Left','Right'),('left','right'),('LEFT','RIGHT')]
seperators =  { '.', ' ', '-', '_' }
...
def sync_names(o):
    ...
    cspy.sync_particle_settings_names(o)
    cspy.sync_mesh_names(o)
    cspy.sync_armature_names(o)


def flip_side_name(from_name):
    t = len(seperators)*len(side_pairs)

    if len(tokens_start) != t or len(tokens_mid) != t or len(tokens_mid2) != t or len(tokens_end) != t:
        tokens_start.clear()
        tokens_mid.clear()
        tokens_mid2.clear()
        tokens_end.clear()
        for sep in seperators:
            for pair in side_pairs:
                tokens_start.append((format_start.format(pair[0], sep), format_start.format(pair[1], sep)))
                tokens_mid.append((format_mid.format(pair[0], sep), format_mid.format(pair[1], sep)))
                tokens_mid2.append((format_mid2.format(pair[0], sep), format_mid2.format(pair[1], sep)))
                tokens_end.append((format_end.format(pair[0], sep), format_end.format(pair[1], sep)))
