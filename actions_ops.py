...
class ACT_BAKE:
    def exec(self, context, action):
        ...
        cspy.actions.select_all_frames(action)
        cspy.actions.sample_fcurve(action)
        cspy.actions.deselect_all_frames(action)
        ...
        for action in bpy.data.actions:            
            ...
            for fc in action.fcurves:
                ...
                for index, key in enumerate(kps):
                    ...
                    cspy.actions.insert_keyframe(master_curve, f, v, needed=False, fast=True, keyframe_type=key.type)
    ...
    def do_execute(self, context):
        ...
        for action in bpy.data.actions:
            if action.name.startswith(context.active_object.name):
                cspy.actions.update_rotations_to_quat(context.active_object, path_check, '')   
