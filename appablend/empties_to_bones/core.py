...
def _initialize_armature():
    ...
    if scn.eb_source_object:
        set_object_active(scn.eb_source_object)

    empties = [e for e in bpy.context.selected_objects if e.type == "EMPTY"]
    for e in empties:
        hierarchy.select_hierarchy(e)
        ...
        if scn.eb_auto_bones_orientation:
            ...
            if child_count == 1:
                ...
                if loc != e_bone.head:  # avoid zero length bone errors
                    print(
                        "{0}: [{1}]".format(
                            emp_child.name, abs((loc - e_bone.tail).magnitude)
                        )
                    )
                    e_bone.tail = loc
            ...
            if not set_tail_to_child:
                ...
                if child_count > 1:
                    ...
                    if empty_primary_axis:
                        e_bone.tail = e_bone.head + (
                            emp_x.normalized() * vec.magnitude * 0.5
                        )
                    else:
                        print(
                            "No primary axis yet, use parent bone vector", e_bone.name
                        )
...
def _create_norot_bones():
    ...
    for edit_bone in current_edit_bones:
        ...
        new_bone = bpy.context.active_object.data.edit_bones.new(
            edit_bone.name + "_NOROT"
        )
        new_bone.head, new_bone.tail, new_bone.roll, new_bone.use_connect = (
            edit_bone.head.copy(),
            edit_bone.tail.copy(),
            edit_bone.roll,
...
def _damped_tracking(bone, bone_NOROT):
    if bone_NOROT.parent:
        ...
        if len(bone_NOROT.children) == 1:
            ...
            child_spines = [
                c
                for c in bone_NOROT.children
                if "spine" in c.name.lower() or "spine" == c.name.lower()
            ]
...
def bake_anim(
    ...
    sk_objects = []
    if (
        shape_keys and _self and action_export_name
    ):  # bake shape keys value for animation export
    ...
    if scn.eb_target_action:
        ...
        target_action_name = (
            scn.eb_target_action_name.replace(".fbx", "")
            .replace(".Fbx", "")
            .replace(".FBX", "")
        )
            ...
            for (f, matrix) in bones_data:
                ...
                if rotation_mode == "QUATERNION":
                    ...
                    for arr_idx, value in enumerate(pbone.rotation_quaternion):
                        store_keyframe(
                            pbone.name, "rotation_quaternion", arr_idx, f, value
                        )

                elif rotation_mode == "AXIS_ANGLE":
                    for arr_idx, value in enumerate(pbone.rotation_axis_angle):
                        store_keyframe(
                            pbone.name, "rotation_axis_angle", arr_idx, f, value
                        )
            ...
            for fc_key, key_values in keyframes.items():
                ...
                if fcurve == None:
                    fcurve = action.fcurves.new(
                        data_path, index=index, action_group=pbone.name
                    )
...
def _initialize_empties(scn, armature):
    ...
    bone_matrices = get_edit_bone_matrices(armature)
    bone_tail_updates = {}
    ...
    for bone_name in bone_tail_updates.keys():
        ...
        # bones.set_world_tail(armature, bone_name, bone_tail_update)
...
def _deconstruct_armature():
    ...
    set_object_active(top_level_empty)
    select_by_names(empties.keys())
    ...
    for child in armature.children:
        hierarchy.delete_hierarchy(child)
