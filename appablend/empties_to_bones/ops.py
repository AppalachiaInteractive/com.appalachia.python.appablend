...
from appablend.common.basetypes.ops import OPS_, OPS_DIALOG
from appablend.common.imports import import_fbx
from appablend.common.utils import data, hierarchy
from appablend.common.utils.common import set_object_active
from appablend.empties_to_bones.core import (_create_armature,
                                             _duplicate_armature, bake_anim,
                                             delete_edit_bone)
...
class EB_OPS_:
    ...
    def execute_deconstruct(self, context):
        ...
        try:
            ...
            empties_to_bones._deconstruct_armature()
            bpy.ops.object.mode_set(mode="OBJECT", toggle=False)
            ...
            if self.clear_empties:
                ...
                for tricky_bone in tricky_bones:
                    ...
                    if root:
                        ...
                        hierarchy.delete_hierarchy(root)
...
class EB_OPS_BAKE:
    bake_type: bpy.props.EnumProperty(
        items=(
            ("ANIM", "Anim", "Bake animation"),
            ("STATIC", "Static", "Bake single pose"),
        ),
        description="Finalize the armature conversion by baking the current pose or animation",
    )
    frame_start: bpy.props.IntProperty(
        default=0, description="First frame to bake from"
    )
    frame_end: bpy.props.IntProperty(default=10, description="Last frame to bake to")
    clear_empties: bpy.props.BoolProperty(
        default=True, description="Delete the empties chain"
    )
    clear_armature: bpy.props.BoolProperty(
        default=True, description="Delete the armature"
    )
...
class EB_OT_deconstruct_duplicate_bake(EB_OPS_BAKE, EB_OPS_, OPS_DIALOG, Operator):
    ...
    def invoke(self, context, event):
        return self.invoke_bake(
            context, event, context.active_object.animation_data.action
        )
...
class EB_OT_process_batch(EB_OPS_BAKE, EB_OPS_, OPS_, Operator):
    ...
    def do_execute(self, context):
        ...
        if scene.eb_target_type == "FILE":
            abs_path = files.abspath(scene.eb_target_file)
            files.append(abs_path)
        else:
            abs_path = files.abspath(scene.eb_target_dir)
            f = files.get_files_in_dir(abs_path, endswith=".fbx", case_sensitive=False)
            files.extend(f)
        ...
        for f in files:
            ...
            import_fbx.import_fbx(filepath=f, automatic_bone_orientation=True)
        ...
        set_object_active(original_active_object)
