...
from appablend.common.utils.common import deselect_all
from appablend.imports.maya import get_maya_import_command
...
class IMPORTS_OT_generate_maya_import_export_commands(OPS_, Operator):
    ...
    def do_execute(self, context):
        ...
        filepaths = [
            path
            for path in files.get_files_in_dir(
                abs_path, "", "", ".fbx", False, s.maya_import_recursive
            )
        ]
...
class IMPORTS_OT_import_directory(OPS_, Operator):
    ...
    def do_execute(self, context):
        ...
        filepaths = [
            path
            for path in files.get_files_in_dir(
                path, "", "", ".fbx", False, s.import_recursive
            )
        ]
