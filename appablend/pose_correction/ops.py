...
class PC_OT_record_reference_from_bone(PC_OP, OPS_, Operator):
    ...
    def bone_execute(self, context, bone, pose_correction):
        ...
        pose_correction.set_ref(
            self.prop_name, val, context.scene.frame_current, context.scene.cursor
        )
...
class PC_OT_correct_pose:
    ...
    def do_poll(cls, context):
        first = (
            DOIF.MODE.IS_POSE(context)
            and DOIF.ACTIVE.POSE_BONE(context)
            and context.scene.frame_current >= context.scene.frame_start
            and context.scene.frame_current <= context.scene.frame_end
        )
    ...
    def do_apply(
        self, context, arm, bone, pose_correction, prop_name, changed_bones, length
    ):
