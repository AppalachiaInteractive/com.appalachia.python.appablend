...
from appablend.pose_correction.core import PoseCorrection
from appablend.pose_correction.ops import (PC_OT_correct_pose_location,
...
from appablend.timeline.ops import (TIMELINE_OT_next_frame,
...
class VIEW_3D_PT_Pose_Correction(_PT_PC_, UI.VIEW_3D.UI.Tool, PT_, bpy.types.Panel):
    ...
    def do_draw(self, context, scene, layout, obj):
        ...
        if bone:
            layout.label(text=bone.name, icon=icons.BONE_DATA)
...
class _PT_PC_AB:
    ...
    def do_draw(self, context, scene, layout, obj):
        ...
        self.draw_pose_correction(
            context, scene, layout, obj, arm, pose, bone, pose_correction
        )
    ...
    def draw_lock(self, layout, alerting, pose_correction, pose, prop_name):
        ...
        row2.prop_search(
            pose_correction, "{0}_lock_bone_name".format(prop_name), pose, "bones"
        )
    ...
    def draw_location_negate(self, layout, alerting, pose_correction, pose):
        ...
        row.alert = alerting
        row.prop_search(
            pose_correction, "location_negate_bone_name", pose, "bones", text="Bone"
        )
    ...
    def draw_transform_negate(self, layout, alerting, pose_correction, pose):
        ...
        row.alert = alerting
        row.prop_search(
            pose_correction, "transform_negate_bone_name", pose, "bones", text="Bone"
        )
    ...
    def draw_operator_buttons(self, layout, operator, alerting, prop_name):
        ...
        row.alert = alerting
        row.operator(
            TIMELINE_OT_prev_frame.bl_idname,
            text="",
            icon=TIMELINE_OT_prev_frame.bl_icon,
        )

        rwall = row.operator(operator, text="", icon=icons.PREV_KEYFRAME)
        rw = row.operator(operator, text="", icon=icons.REW)
        cur = row.operator(operator)
        ff = row.operator(operator, text="", icon=icons.FF)
        ffall = row.operator(operator, text="", icon=icons.NEXT_KEYFRAME)

        row.operator(
            TIMELINE_OT_next_frame.bl_idname,
            text="",
            icon=TIMELINE_OT_next_frame.bl_icon,
        )

        rwall.loop, rwall.advance, rwall.forward, rwall.prop_name = (
            True,
            True,
            False,
            prop_name,
        ...
        cur.loop, cur.advance, cur.forward, cur.prop_name = (
            False,
            False,
            False,
            prop_name,
        ...
        ffall.loop, ffall.advance, ffall.forward, ffall.prop_name = (
            True,
            True,
            True,
            prop_name,
    ...
    def draw_pose_correction(
        self, context, scene, layout, obj, arm, pose, bone, pose_correction
    ):
        ...
        reference_frame = getattr(
            pose_correction, "{0}_reference_frame".format(prop_name), 0
        )
        ...
        self.draw_operator_buttons(
            layout, self.correct_op, scene.frame_current == reference_frame, prop_name
        )


class VIEW_3D_PT_Pose_Correction_01_LOC(
    _PT_PC_AB, UI.VIEW_3D.UI.Tool, PT_, bpy.types.Panel
):
    bl_label = "Location"
...
class VIEW_3D_PT_Pose_Correction_02_ROT(
    _PT_PC_AB, UI.VIEW_3D.UI.Tool, PT_, bpy.types.Panel
):
    bl_label = "Rotation"
...
class VIEW_3D_PT_Pose_Correction_04_TRN(
    _PT_PC_AB, UI.VIEW_3D.UI.Tool, PT_, bpy.types.Panel
):
    bl_label = "Transform"
    bl_idname = "VIEW_3D_PT_Pose_Correction_04_TRN"
    bl_icon = icons.CON_TRANSFORM
    bl_parent_id = VIEW_3D_PT_Pose_Correction.bl_idname
