...
import bpy
from appablend.animation_metadata.enums import AM_ENVS
from appablend.common.core.enums import *
...
class AnimationMetadataBase:
    ...
    environment_start: bpy.props.EnumProperty(
        name="Environment", items=_get_env_start_enum, default=0
    )
    changes_environment: bpy.props.BoolProperty(name="Changes?", default=False)
    environment_end: bpy.props.EnumProperty(
        name="End", items=_get_env_end_enum, default=0
    )

    category_start: bpy.props.EnumProperty(
        name="Category", items=_get_category_start_enum, default=0
    )
    changes_category: bpy.props.BoolProperty(name="Changes?", default=False)
    category_end: bpy.props.EnumProperty(
        name="End", items=_get_category_end_enum, default=0
    )
    ...
    pose_start: bpy.props.EnumProperty(
        name="Pose", items=_get_pose_start_enum, default=0
    )
    ...
    substate: bpy.props.EnumProperty(
        name="Substate", items=_get_substate_enum, default=0
    )

    orientation_end: bpy.props.FloatProperty(
        name="Orientation",
        default=0.0,
        min=-math.pi * 2,
        max=math.pi * 2,
        subtype=ST_FloatProperty.Subtypes.ANGLE,
        unit=ST_FloatProperty.Units.ROTATION,
    )

    direction: bpy.props.EnumProperty(
        name="Direction", items=_get_direction_enum, default=0
    )
    ...
    def _get_environment(self):
        return (
            ...
            self.environment_end if self.changes_environment else "",
        )
    ...
    def get_action_name(self):
        ...
        action_name = action_name.replace(
            NameTemplate.TK_RMIP, "RM" if self.motion else "IP"
        )
        action_name = action_name.replace(NameTemplate.TK_VARI, str(self.variation))
        action_name = action_name.replace(
            NameTemplate.TK_DFFL, "FL" if self.mirrored else "DF"
        )
    ...
    def draw_basic(self, context, col, prop, icon=icons.CANCEL):
        row1 = col.row(align=True)
        row1.prop(self, prop, icon=icon)

    def draw_toggle(self, context, col, prop, icon=icons.CANCEL):
        row1 = col.row(align=True)
        row1.prop(self, prop, icon=icon, toggle=True)

    def draw_start_change_end(
        self, context, col, key_start, key_change="", key_end="", icon=icons.CANCEL
    ):
        key_change = key_start if key_change == "" else key_change
        key_end = key_start if key_end == "" else key_end

        start_prop_name = (
            key_start if "_start" in key_start else "{0}_start".format(key_start)
        )
        ...
        row1b.prop(
            ...
            toggle=True,
            text="",
            icon=icons.CHECKBOX_HLT if changes else icons.CHECKBOX_DEHLT,
        )
        row1c = row1.split()
    ...
    def draw(self, context, layout, check_op):
        ...
        row = col.row(align=True)
        self.draw_basic(context, row, "actor", icons.MONKEY)
        self.draw_basic(context, row, "mask", icons.MOD_MASK)

        self.draw_start_change_end(context, col, "environment", icon=icons.WORLD_DATA)
        self.draw_start_change_end(
            context, col, "category", icon=icons.OUTLINER_OB_GROUP_INSTANCE
        )
        self.draw_start_change_end(
            context, col, "pose", icon=icons.OUTLINER_OB_ARMATURE
        )
        self.draw_basic(context, col, "substate", icon=icons.MOD_ARMATURE)

        row = col.row(align=True)
        self.draw_basic(context, row, "direction", icons.ORIENTATION_CURSOR)
        self.draw_basic(context, row, "orientation_end", icons.ORIENTATION_GIMBAL)

        row = self.get_row(col)
        self.draw_basic(context, row, "variation", icon=icons.SEQUENCE)
        self.draw_basic(context, row, "stage", icons.FILE_REFRESH)

        row = self.get_row(col)
        self.draw_toggle(context, row, "motion", icon=icons.ORIENTATION_GLOBAL)
        self.draw_toggle(context, row, "mirrored", icon=icons.MOD_MIRROR)

        col.separator()
        col.operator(check_op, icon=icons.VIEWZOOM)
