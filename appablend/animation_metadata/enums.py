...
class AM_ENUM:
    @classmethod
    def create_enum_dict(cls, enum_tuples) -> Tuple[List[str], str, Callable, Callable]:
        enums = []
        default = ""
    ...
    def __init__(self):
        ...
        (
            ...
            self.GET_ENUM,
            self.GET_DEFAULT,
        ) = self.create_enum_dict(self.VALS)
...
class AM_Pose:
    def __init__(self):
        self.substate_enum: AM_ENUM = AM_ENUM()
        self.direction_enum: AM_ENUM = AM_ENUM()


class AM_Category:
    def __init__(self):
        self.mask_enum: AM_ENUM = AM_ENUM()
        self.stage_enum: AM_ENUM = AM_ENUM()
        self.pose_enum: AM_ENUM = AM_ENUM()
        self.poses: Dict[str, AM_Pose] = {}


class AM_Environment:
    def __init__(self):
        self.category_enum: AM_ENUM = AM_ENUM()
        self.categories: Dict[str, AM_Category] = {}


class PoseMetadata:
    def __init__(
        self,
        all_substates: List[Tuple[str, str]],
        all_directions: List[Tuple[str, str]],
    ):
        self.all_substates: List[Tuple[str, str]] = all_substates
        self.all_directions: List[Tuple[str, str]] = all_directions
...
class EnvMetadata:
    def __init__(
        ...
        all_masks: List[Tuple[str, str]],
        all_stages: List[Tuple[str, str]],
        all_poses: List[Dict[str, PoseMetadata]],
    ):
        self.all_categories: List[Tuple[str, str]] = all_categories
        self.all_masks: List[Tuple[str, str]] = all_masks
        self.all_stages: List[Tuple[str, str]] = all_stages
        self.all_poses: List[Dict[str, PoseMetadata]] = all_poses


class AM_ENVS:
    def _get_env_set(self):
        _E, _C, _M, _S, _P, _SS, _D = (
            _ENVIRONMENT,
            _CATEGORY,
            _MASK,
            _STAGE,
        ...
        #EnvMetadata = namedtuple(
        #    "EnvMetadata", "all_categories all_masks all_stages all_poses"
        #)
        #PoseMetadata = namedtuple("PoseMetadata", "all_substates all_directions")
        
        environments: Dict[str, EnvMetadata] = {}
            ...
            [
                ...
                {  # _C.VOCL
                    _P.NONE: PoseMetadata(
                        [_SS.CALL, _SS.BARK, _SS.NTFY, _SS.GRWL, _SS.WHNE, _SS.YELP],
                        [_D.NONE],
                    ),
                },
                {  # _C.EMOT
                ...
                {  # _C.SENS
                    _P.NONE: PoseMetadata(
                        [_SS.LSTN, _SS.LOOK, _SS.SMEL],
                        [_D.LEFT, _D.RGHT, _D.FRNT, _D.BACK, _D.UPWR, _D.DOWN],
                    ),
                },
            ],
        )
        ...
        environments[_E.AQUA] = EnvMetadata(
            ...
            [
                {  # _C.IDLE
                    _P.IDLF: PoseMetadata(
                        [_SS.WAIT], [_D.NONE]
                    ),  # IDLF = ('Float','IDLF')
                },
                {  # _C.MOVE
                    _P.SWIM: PoseMetadata(
                        [_SS.MOVE],
                        [_D.LEFT, _D.RGHT, _D.FRNT, _D.BACK, _D.UPWR, _D.DOWN],
                    ),  # SWIM = ('Swim','SWIM')
                },
                ...
                {  # _C.CMBT
                    _P.IDLF: PoseMetadata(
                        [_SS.IMPT, _SS.DIEC], [_D.LEFT, _D.RGHT, _D.FRNT, _D.BACK]
                    ),
                    _P.SWIM: PoseMetadata(
                        [_SS.IMPT, _SS.DIEC, _SS.BITE, _SS.CLAW],
                        [_D.LEFT, _D.RGHT, _D.FRNT, _D.BACK],
                    ),  # DIEC = ('Death (Combat)', 'DIEC')
                },
                {  # _C.DEAD
                    _P.DEDW: PoseMetadata(
                        [_SS.NONE], [_D.NONE]
                    ),  # DEDW = ('Dead (Aquatic)','DEDW')
                },
            ],
        ...
        environments[_E.SURF] = EnvMetadata(
            ...
            [
                ...
                {  # _C.CMBT
                    ...
                    _P.RSTW: PoseMetadata(
                        [_SS.IMPT, _SS.DIEC], [_D.LEFT, _D.RGHT, _D.FRNT, _D.BACK]
                    ),
                    _P.SLPW: PoseMetadata(
                        [_SS.IMPT, _SS.DIEC], [_D.LEFT, _D.RGHT, _D.FRNT, _D.BACK]
                    ),
                    _P.IDLT: PoseMetadata(
                        [_SS.IMPT, _SS.DIEC, _SS.BITE, _SS.CLAW],
                        [_D.LEFT, _D.RGHT, _D.FRNT, _D.BACK],
                    ),
                    _P.PDDL: PoseMetadata(
        ...
        environments[_E.LAND] = EnvMetadata(
            ...
            [
                ...
                {  # _C.ACTV
                    ...
                    _P.RSTG: PoseMetadata(
                        [_SS.CLEN, _SS.EATT, _SS.GROO, _SS.REST], [_D.NONE]
                    ),
                    _P.SLPG: PoseMetadata([_SS.SLEP], [_D.NONE]),
                },
                {  # _C.MOVE
                    _P.BIPD: PoseMetadata(
                        [
                            _SS.MOVE,
                        ],
                        [
                    ...
                    _P.WWLK: PoseMetadata(
                        [_SS.MOVE], [_D.LEFT, _D.RGHT, _D.FRNT, _D.BACK]
                    ),
                    _P.WFST: PoseMetadata(
                        [_SS.MOVE, _SS.JUMP],
                        [
                            _D.LEFT,
                            _D.RGHT,
                ...
                {  # _C.CMBT
                    ...
                    _P.SITG: PoseMetadata(
                        [_SS.IMPT, _SS.DIEC], [_D.LEFT, _D.RGHT, _D.FRNT, _D.BACK]
                    ),
                    _P.SLPG: PoseMetadata(
                        [_SS.IMPT, _SS.DIEC], [_D.LEFT, _D.RGHT, _D.FRNT, _D.BACK]
                    ),
                    _P.STLK: PoseMetadata(
                        [_SS.IMPT, _SS.DIEC], [_D.LEFT, _D.RGHT, _D.FRNT, _D.BACK]
                    ...
                    _P.IDLW: PoseMetadata(
                        [_SS.IMPT, _SS.DIEC, _SS.DIEI],
                        [_D.LEFT, _D.RGHT, _D.FRNT, _D.BACK],
                    ),
                    _P.WWLK: PoseMetadata(
                        [_SS.IMPT, _SS.DIEC, _SS.DIEI],
                    ...
                    _P.CMBT: PoseMetadata(
                        ...
                        [_D.NONE, _D.LEFT, _D.RGHT, _D.FRNT, _D.BACK],
                    ),
                    _P.CMTW: PoseMetadata(
                        [_SS.IMPT, _SS.DIEC, _SS.DIEI, _SS.BITE, _SS.HEAD, _SS.HORN],
                        [_D.LEFT, _D.RGHT, _D.FRNT, _D.BACK],
                    ),
                    _P.CMTT: PoseMetadata(
                        [
                            _SS.IMPT,
                            _SS.DIEC,
                            _SS.BITE,
        ...
        environments[_E.TREE] = EnvMetadata(
            ...
            [
                ...
                {  # _C.ACTV
                    _P.IDLT: PoseMetadata(
                        [_SS.CLEN, _SS.DEFE, _SS.EATT, _SS.GROO, _SS.SCTR], [_D.NONE]
                    ),
                    _P.SITT: PoseMetadata(
                        [_SS.CLEN, _SS.DEFE, _SS.EATT, _SS.GROO], [_D.NONE]
                    ),
                    _P.RSTT: PoseMetadata([_SS.REST, _SS.CLEN, _SS.GROO], [_D.NONE]),
                    _P.SLPT: PoseMetadata([_SS.SLEP], [_D.NONE]),
                },
                {  # _C.MOVE
                    _P.CLMB: PoseMetadata(
                        [_SS.MOVE, _SS.WAIT],
                        [
                            _D.LEFT,
                            _D.RGHT,
                ...
                {  # _C.CMBT
                    ...
                    _P.RSTT: PoseMetadata(
                        [_SS.IMPT, _SS.DIEC], [_D.LEFT, _D.RGHT, _D.FRNT, _D.BACK]
                    ),
                    _P.SLPT: PoseMetadata(
                        [_SS.IMPT, _SS.DIEC], [_D.LEFT, _D.RGHT, _D.FRNT, _D.BACK]
                    ),
                    _P.CLMB: PoseMetadata(
                        [_SS.IMPT, _SS.DIEC, _SS.BITE, _SS.CLAW],
        ...
        environments[_E.AERL] = EnvMetadata(
            ...
            [
                ...
                {  # _C.CMBT
                    ...
                    _P.GLDE: PoseMetadata(
                        [_SS.IMPT, _SS.DIEC, _SS.BITE, _SS.CLAW, _SS.GRAB],
                        [_D.NONE, _D.LEFT, _D.RGHT, _D.FRNT, _D.BACK],
                    ),
                    _P.DIVE: PoseMetadata(
                        [_SS.IMPT, _SS.DIEC], [_D.LEFT, _D.RGHT, _D.FRNT, _D.BACK]
    ...
    def _set_enum_only(self, pp: AM_ENUM, pkv):
        pp.VALS[pkv[1]] = pkv[0]
        ...
        for env_key in environments.keys():
            ...
            for index, category_key in enumerate(env_data.all_categories):
                ...
                if length_categories != length_masks:
                    print(
                        "[{0}]: Length mismatch: Categories: {1}  vs. Masks: {2}".format(
                            env_key, length_categories, length_masks
                        )
                    )
                if length_categories != length_stages:
                    print(
                        "[{0}]: Length mismatch: Categories: {1}  vs. Stages: {2}".format(
                            env_key, length_categories, length_stages
                        )
                    )
                if length_categories != length_poses:
                    print(
                        "[{0}]: Length mismatch: Categories: {1}  vs. Poses: {2}".format(
                            env_key, length_categories, length_poses
                        )
                    )
