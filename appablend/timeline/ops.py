...
from appablend.common.nla import get_selected_strips
...
class TIMELINE_OT_guess_previous_start(TIMELINE_OP, OPS_, Operator):
    ...
    def do_execute(self, context):
        ...
        context.scene.frame_start = get_previous_notable_frame_from_start(
            context, action
        )
...
class TIMELINE_OT_clear:
    ...
    def do_poll(cls, context):
        s = context.scene
        return (
            DOIF.ACTIVE.HAS.ACTION(context)
            and s.frame_current >= s.frame_start
            and s.frame_current <= s.frame_end
        )
