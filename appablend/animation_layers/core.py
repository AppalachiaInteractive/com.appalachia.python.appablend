...
def check_handler(self, context):
    ...
    if (
        ...
        or not hasattr(anim_data, "action")
    ):
        return

    if (
        obj.select_get() == False
        or not hasattr(anim_data, "nla_tracks")
        or not obj.als.track_list
    ):
        return
    ...
    if bpy.context.active_operator is not None:
        if bpy.context.active_operator.name in [
            "Add Action Strip",
            "Delete Strips",
            "Move Channels",
        ]:
            ...
            return
        if (
            bpy.context.active_operator.name in ["Transform", "Delete Keyframes"]
            and obj.als.edit_all_keyframes
        ):
            edit_all_keyframes()
...
def base_initial_key(obj, anim_data):
    ...
    for fcu in anim_data.action.fcurves:
        ...
        for i, track in enumerate(anim_data.nla_tracks[0:-1]):
            if (
                len(track.strips) != 1
                or i == obj.track_list_index
                or track.strips[0].action is None
                or track.mute
            ):
                continue
            if track.strips[0].blend_type != "ADD":
                continue
            fcu_track = track.strips[0].action.fcurves.find(
                fcu.data_path, index=fcu.array_index
            )
            if fcu_track is None:
                continue
            if (
                fcu_track.data_path == fcu.data_path
                and fcu_track.array_index == fcu.array_index
            ):
        ...
        if obj.type == "ARMATURE" and fcu.data_path not in transform_types:
            ...
            if transform in dir(obj.pose.bones[bone]):
                if (
                    transform in transform_types
                ):  # if it's a vector transform then get the value with array index
...
def check_fcurves(AL_item, obj):
    ...
    if (
        anim_data.nla_tracks[obj.track_list_index].strips[0].blend_type
        not in blend_types
    ):
        return
...
def check_influence(track):
    ...
    global keys
    if (
        track.strips[0].fcurves[0].mute
        or not len(track.strips[0].fcurves[0].keyframe_points)
        or bpy.context.scene.tool_settings.use_keyframe_insert_auto
    ):
        if "keys" in globals():
            del keys  # type: ignore
    ...
    if keys != [tuple(key.co) for key in track.strips[0].fcurves[0].keyframe_points]:  # type: ignore
...
def update_action(obj):
    ...
    if len(anim_data.nla_tracks) <= 1:
        return
    if (
        anim_data.action == anim_data.nla_tracks[obj.track_list_index].strips[0].action
        and anim_data.action == anim_data.nla_tracks[-1].strips[0].action
    ):
        return
    if obj.Anim_Layers[obj.track_list_index].lock:
        return
...
def register_layers(nla_tracks):
    ...
    for i, track in enumerate(nla_tracks[:-1]):
        ...
        if (
            len(track.strips) != 1
            or track.strips[0].type == "META"
            and len(obj.Anim_Layers) > i + 1
        ):
...
def layer_lock(self, context):
    ...
    if not self.lock:
        if (
            len(nla_tracks[index].strips) != 1
            or nla_tracks[index].strips[0].type == "META"
        ):  # and not self.lock:
            self.lock = True
...
def hide_view_all_keyframes(obj, anim_data):
    ...
    for i, layer in enumerate(obj.Anim_Layers):
        ...
        if (
            not obj.als.edit_all_keyframes and not fcu.group.lock
        ):  # lock the groups if edit is not selected
            fcu.group.lock = True

        if bpy.context.area:
            if (
                bpy.context.area.type != "GRAPH_EDITOR"
            ):  # hide the channels when using graph editor
                return
...
def fcurve_bones_path(obj, fcu):
    ...
    if obj.als.only_selected_bones and obj.mode == "POSE":
        selected_bones_path = [
            bone.path_from_id() for bone in bpy.context.selected_pose_bones
        ]
...
def edit_all_keyframes():
    ...
    for i, layer in enumerate(obj.Anim_Layers):  # look for the Anim Layers fcurve
        ...
        if (
            len(fcu_layers[fcu.data_path]) != len(fcu.keyframe_points)
            and bpy.context.active_operator.name == "Delete Keyframes"
        ):
            keyframes = store_keyframes(fcu, [])
            del_keys = list(set(fcu_layers[fcu.data_path]) - set(keyframes))
            for fcurve in (
                anim_data.nla_tracks[i].strips[0].action.fcurves
        ...
        for (
            ...
            fcu.keyframe_points
        ):  # creates dictionary of the old key frame values with their difference
...
def view_all_keyframes(self, context):
    ...
    if (
        len(anim_data.nla_tracks[:-1]) != len(obj.Anim_Layers)
        or anim_data.action is None
    ):
        return
    ...
    for i, track in enumerate(anim_data.nla_tracks[:-1]):
        ...
        if fcu:  # remove all the fcurves/channels in the group and mark as removed
            if fcu.group.name == "Anim Layers":
                ...
                break
    if (
        not self.view_all_keyframes
    ):  # If the option is uncheck then finish edit and return
    ...
    for i, track in enumerate(anim_data.nla_tracks[:-1]):
        if (
            i == obj.track_list_index
            or track.strips[0].action is None
            or not len(track.strips[0].action.fcurves)
            or obj.Anim_Layers[i].lock
        ):
            continue
        # create a new fcurve with the name of the track
        fcu_layer = anim_data.action.fcurves.new(
            track.name, index=i, action_group="Anim Layers"
        )
        fcu_layer.update()
...
class AnimLayersSettings(bpy.types.PropertyGroup):
    ...
    track_list: bpy.props.BoolProperty(
        name="Turn Animation Layers On",
        description="View all actions UI",
        default=False,
        options={"HIDDEN"},
        update=turn_animlayers_on,
    )
    linked: bpy.props.BoolProperty(
        name="Linked",
        description="Duplicate a layer with a linked action",
        default=False,
        options={"HIDDEN"},
    )
    smartbake: bpy.props.BoolProperty(
        name="Smart Bake",
        description="Stay with the same amount of keyframes after merging and baking",
        default=False,
        options={"HIDDEN"},
    )
    onlyselected: bpy.props.BoolProperty(
        name="Only selected Bones",
        description="Bake only selected Armature controls",
        default=False,
        options={"HIDDEN"},
    )
    clearconstraints: bpy.props.BoolProperty(
        name="Clear constraints",
        description="Clear constraints during bake",
        default=False,
        options={"HIDDEN"},
    )
    mergefcurves: bpy.props.BoolProperty(
        name="Merge Fcurve modifiers",
        description="Include Fcurve modifiers in the bake",
        default=False,
        options={"HIDDEN"},
    )
    view_all_keyframes: bpy.props.BoolProperty(
        name="View",
        description="View keyframes from multiple layers, use lock and mute to exclude layers",
        default=False,
        update=view_all_keyframes,
    )
    edit_all_keyframes: bpy.props.BoolProperty(
        name="Edit",
        description="Edit keyframes from multiple layers",
        default=False,
        update=unlock_edit_keyframes,
    )
    only_selected_bones: bpy.props.BoolProperty(
        name="Only Selected Bones",
        description="Edit and view only selected bones",
        default=False,
        update=only_selected_bones,
    )
    view_all_type: bpy.props.EnumProperty(
        name="Type",
        description="Select visibiltiy type of keyframes",
        update=view_all_keyframes,
        items=[
    ...
    baketype: bpy.props.EnumProperty(
        ...
        items=[
            ("AL", "Anim Layers", "Use Animation Layers Bake", 0),
            ("BLENDER", "Blender Bake", "Use Blender internal Bake", 1),
        ],
    )
    direction: bpy.props.EnumProperty(
        ...
        items=[
            ("UP", "Up", "Merge upwards", "TRIA_UP", 1),
            ("DOWN", "Down", "Merge downwards", "TRIA_DOWN", 0),
            ("ALL", "All", "Merge all layers"),
        ],
    )
    operator: bpy.props.EnumProperty(
        ...
        description="Type of bake",
        items=[
            ("NEW", "New Baked Layer", "Bake into a New Layer", "NLA", 1),
            ("MERGE", "Merge", "Merge Layers", "NLA_PUSHDOWN", 0),
        ],
    )
...
class AnimLayersItems(bpy.types.PropertyGroup):
    ...
    mute: bpy.props.BoolProperty(
        name="Mute",
        description="Mute Animation Layer",
        default=False,
        options={"HIDDEN"},
        update=layer_mute,
    )
    lock: bpy.props.BoolProperty(
        name="Lock",
        description="Lock Animation Layer",
        default=False,
        options={"HIDDEN"},
        update=layer_lock,
    )
    solo: bpy.props.BoolProperty(
        name="Solo",
        description="Solo Animation Layer",
        default=False,
        options={"HIDDEN"},
        update=layer_solo,
    )
...
class AnimLayersObjects(bpy.types.PropertyGroup):
    object: bpy.props.PointerProperty(
        name="object",
        description="objects with animation layers turned on",
        type=bpy.types.Object,
    )
    fcurves: bpy.props.IntProperty(
        name="fcurves", description="helper to check if fcurves are changed", default=0
    )
