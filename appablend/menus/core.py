...
mc_section_type_list = [
    (
        "DEFAULT",
        "Default",
        "A simple collection of properties that can be added right clicking on fields -> Add Property to the Menu",
    ),
    (
        "COLLECTION",
        "Collection List",
        "Right clicking on them in the Outliner, you can add collections whose elements can be shown/hidden in the Menu. Only one collection will be shown at the same time.\nIdeal for: Outfit lists",
        "OUTLINER_COLLECTION",
        1,
    ),
]
...
class MC_Settings(bpy.types.PropertyGroup):
    ...
    mss_name: bpy.props.StringProperty(
        name="Name",
        description="Name of the menu.\nChoose the name of the menu to be shown before the properties",
        default="Object: ",
    )
    mss_obj_name: bpy.props.BoolProperty(
        name="Show the Object Name",
    ...
    mss_path_replace_old: bpy.props.StringProperty(
        name="Replace Old", description="Replace...", default=""
    )
    mss_path_replace_new: bpy.props.StringProperty(
        name="Replace New", description="Replace with...", default=""
    )

    # Edit mode properties
    em_fixobj: bpy.props.BoolProperty(
        name="Pin Object",
...
class MCSectionItem(bpy.types.PropertyGroup):
    ...
    collections: bpy.props.CollectionProperty(
        name="Section Collection List", type=MCCollectionItem
    )
    collections_list: bpy.props.EnumProperty(
        name="Section Collection List",
        items=mc_collections_list,
        update=mc_collections_list_update,
    )
    collections_global_smoothcorrection: bpy.props.BoolProperty(
        name="Smooth Correction",
        default=True,
        update=mc_collections_global_options_update,
    )
    collections_global_shrinkwrap: bpy.props.BoolProperty(
        name="Shrinkwrap", default=True, update=mc_collections_global_options_update
    )
    collections_global_mask: bpy.props.BoolProperty(
        name="Mask", default=True, update=mc_collections_global_options_update
    )
    collections_global_normalautosmooth: bpy.props.BoolProperty(
    ...
    outfit_body: bpy.props.PointerProperty(
        name="Outfit Body",
        description="The masks of this object will be switched on/off depending on which elements of the collections visibility",
...
class MCPropertyItem(bpy.types.PropertyGroup):
    ...
    icon: bpy.props.EnumProperty(
        name="Property Icon", default="NONE", items=mc_icon_list
    )
