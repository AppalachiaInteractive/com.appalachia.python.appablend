...
from appablend.common.utils.common import dump, split_path
from appablend.menus.core import *
        ...
        if hasattr(context, "button_prop"):
            ...
            if obj.mc_enable:
                ...
                if mc_add_property_item(obj.mc_properties, [name, rna, path]):
                    self.report(
                        {"INFO"},
                        "Menu Creator - Property added to the '" + obj.name + "' menu.",
                    )
                else:
                    self.report(
                        {"WARNING"},
                        "Menu Creator - Property of '"
                        + obj.name
                        + "' was already added.",
                    )
                ...
                self.report(
                    {"ERROR"},
                    "Menu Creator - Can not add property '"
                    + obj.name
                    + "'. No menu has been initialized.",
                )
...
class MC_AddCollection(OPS_, Operator):
    ...
    def do_execute(self, context):
        ...
        if i:
            ...
            self.report(
                {"INFO"},
                "Menu Creator - Collection has been added to section '"
                + self.section
                + "'.",
            )
        else:
            self.report(
                {"WARNING"},
                "Menu Creator - Collection was already added to section '"
                + self.section
                + "'.",
            )
...
class MC_ReplacePath(OPS_, Operator):
    ...
    def do_execute(self, context):
        ...
        self.report(
            {"INFO"}, "Menu Creator - '" + obj.name + "' path has been replaced."
        )
...
class MC_PropertySettings(OPS_, Operator):
    ...
    name: bpy.props.StringProperty(
        name="Name", description="Choose the name of the property"
    )
...
class MC_AddSection(OPS_, Operator):
    ...
    name: bpy.props.StringProperty(
        name="Name", description="Choose the name of the section"
    )
    icon: bpy.props.EnumProperty(
        name="Icon",
        description="Choose the icon.\nNote that the icon name MUST respect Blender convention. All the icons can be found in the Icon Viewer default Blender addon",
    ...
    type: bpy.props.EnumProperty(
        name="Type", description="Choose the section type", items=mc_section_type_list
    )
    ...
    def do_execute(self, context):
        ...
        if self.name != "":
            ...
            if i:
                ...
                self.report(
                    {"INFO"}, "Menu Creator - Section '" + self.name + "' created."
                )
            else:
                self.report(
                    {"WARNING"},
                    "Menu Creator - Cannot create a section with same name.",
                )

        else:
            self.report(
                {"ERROR"}, "Menu Creator - Cannot create a section with this name."
            )
...
class MC_SectionSettings(OPS_, Operator):
    ...
    type: bpy.props.EnumProperty(
        name="Type",
        description="The Section type can not be changed after creation",
        items=mc_section_type_list,
    )

    # COLLECTION type settings
    collections_enable_global_smoothcorrection: bpy.props.BoolProperty(
        name="Enable Global Smooth Correction"
    )
    collections_enable_global_shrinkwrap: bpy.props.BoolProperty(
        name="Enable Global Shrinkwrap"
    )
    collections_enable_global_mask: bpy.props.BoolProperty(name="Enable Global Mask")
    collections_enable_global_normalautosmooth: bpy.props.BoolProperty(
        name="Enable Global Normal Auto Smooth"
    )
    # Outfit variant
    outfit_enable: bpy.props.BoolProperty(
        name="Outfit",
        description="With this option a Body entry will be added to the Section. This Body's masks will be enabled when elements of the collections are shown, and viceversa, if the masks are called the same name as the element of the collection",
    )

    name_edit: bpy.props.StringProperty(
        name="Name", description="Choose the name of the section"
    )
    ...
    def do_execute(self, context):
        ...
        if i >= 0:
            ...
            sec_obj[
                i
            ].collections_enable_global_smoothcorrection = (
                self.collections_enable_global_smoothcorrection
            )
            sec_obj[
                i
            ].collections_enable_global_shrinkwrap = (
                self.collections_enable_global_shrinkwrap
            )
            sec_obj[
                i
            ].collections_enable_global_mask = self.collections_enable_global_mask
            sec_obj[
                i
            ].collections_enable_global_normalautosmooth = (
                self.collections_enable_global_normalautosmooth
            )
    ...
    def invoke(self, context, event):
        ...
        self.collections_enable_global_smoothcorrection = sec_obj[
            self.ID
        ].collections_enable_global_smoothcorrection
        self.collections_enable_global_shrinkwrap = sec_obj[
            self.ID
        ].collections_enable_global_shrinkwrap
        self.collections_enable_global_mask = sec_obj[
            self.ID
        ].collections_enable_global_mask
        self.collections_enable_global_normalautosmooth = sec_obj[
            self.ID
        ].collections_enable_global_normalautosmooth
...
class MC_CollectionObjectVisibility(OPS_, Operator):
    ...
    def do_execute(self, context):
        ...
        bpy.data.objects[self.obj].hide_viewport = not bpy.data.objects[
            self.obj
        ].hide_viewport
        bpy.data.objects[self.obj].hide_render = not bpy.data.objects[
            self.obj
        ].hide_render
        ...
        if sec_obj[i].outfit_enable:
            for modifier in sec_obj[i].outfit_body.modifiers:
                if (
                    modifier.type == "MASK"
                    and self.obj in modifier.name
                    and sec_obj[i].collections_global_mask
                ):
                    modifier.show_viewport = not bpy.data.objects[
                        self.obj
                    ].hide_viewport
...
class MC_InitialConfiguration(OPS_, Operator):
    ...
    def do_execute(self, context):
        ...
        self.report(
            {"INFO"}, "Menu Creator - Menu for '" + obj.name + "' successfully created."
        )
