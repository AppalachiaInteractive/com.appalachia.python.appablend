...
from appablend.menus.ops import *
...
def mc_collection_menu(self, context):
    ...
    self.layout.menu(OUTLINER_MT_Collection_Menu.bl_idname)  # type: ignore
    ...
    def draw(self, context):
        ...
        for sec in obj.mc_sections:
            if sec.type == "COLLECTION":
                layout.operator(
                    MC_AddCollection.bl_idname, text=sec.name
                ).section = sec.name
                no_col_sec = False
        ...
        if mc_col_len > 0:
            ...
            for sec in sorted(mcs_col, key=mc_sec_ID):
                ...
                if sec.type == "DEFAULT":
                    ...
                    if (sec_empty and sec.name == "Unsorted") or (
                        not obj.mc_edit_enable and not sec_empty and sec_hidden
                    ):
                        continue
                    else:
                        row = layout.row(align=False)
                        ...
                        if obj.mc_edit_enable:
                            ...
                            if sec.name != "Unsorted":
                                ssett_button = row.operator(
                                    "ops.mc_sectionsettings",
                                    icon="PREFERENCES",
                                    text="",
                                )
                            ...
                            sup_button = row2.operator(
                                "ops.mc_swapsections", icon="TRIA_UP", text=""
                            )
                            sup_button.mod = True
                            sup_button.name = sec.name
                            sup_button.icon = sec.icon
                            sdown_button = row2.operator(
                                "ops.mc_swapsections", icon="TRIA_DOWN", text=""
                            )
                        ...
                        if sec_empty and sec.name != "Unsorted":
                            ...
                            row.operator(
                                "ops.mc_deletesection", text="", icon="X"
                            ).name = sec.name
                    ...
                    for el in mc_col:
                        ...
                        if el.section == sec.name:
                            ...
                            if obj.mc_edit_enable:
                                ...
                                sett_button = row.operator(
                                    "ops.mc_propsettings", icon="PREFERENCES", text=""
                                )
                                ...
                                row2 = row.row(align=True)
                                up_button = row2.operator(
                                    "ops.mc_swapprops", icon="TRIA_UP", text=""
                                )
                                up_button.mod = True
                                up_button.name = el.name
                                up_button.path = el.path
                                up_button.id = el.id
                                down_button = row2.operator(
                                    "ops.mc_swapprops", icon="TRIA_DOWN", text=""
                                )
                                ...
                                del_button = row.operator(
                                    "ops.mc_removeproperty", icon="X", text=""
                                )
                    ...
                    if obj.mc_edit_enable:
                        ...
                        ssett_button = row.operator(
                            "ops.mc_sectionsettings", icon="PREFERENCES", text=""
                        )
                        ssett_button.name = sec.name
                        ssett_button.icon = sec.icon
                        ssett_button.type = sec.type
                        ...
                        if sec.outfit_enable:
                            box = layout.box()
                            box.prop(
                                sec, "outfit_body", text="Body", icon="OUTLINER_OB_MESH"
                            )
                    ...
                    if len(sec.collections) > 0:
                        ...
                        if len(bpy.data.collections[sec.collections_list].objects) > 0:
                            for obj2 in bpy.data.collections[
                                sec.collections_list
                            ].objects:
                                row = box2.row()
                                ...
                                if obj2.hide_viewport:
                                    vop = row.operator(
                                        "ops.mc_colobjvisibility",
                                        text=obj2.name,
                                        icon="OUTLINER_OB_" + obj2.type,
                                    )
                                    vop.obj = obj2.name
                                    vop.sec = sec.name
                                else:
                        ...
                        if (
                            sec.collections_enable_global_smoothcorrection
                            or sec.collections_enable_global_shrinkwrap
                            or sec.collections_enable_global_mask
                            or sec.collections_enable_global_normalautosmooth
                        ):
                            box.label(
                                text=sec.name + " Global Properties", icon="MODIFIER"
                            )
                            box2 = box.box()
...
class MC_PT_Settings_Panel(bpy.types.Panel, PT_, UI.VIEW_3D.UI.Menu):
    ...
    def do_draw(self, context, scene, layout, obj):
        ...
        box.operator(
            "ops.mc_cleanpropobj", text="Reset Object", icon="ERROR"
        ).reset = True
        # box.operator('ops.mc_cleanprop', text="Clean all objects")
        box.operator(
            "ops.mc_cleanprop", text="Reset All Objects", icon="ERROR"
        ).reset = True
...
def register():
    ...
    bpy.types.Object.mc_edit_enable = bpy.props.BoolProperty(
        name="Edit Mode",
        default=False,
        description="Enable edit mode in this menu.\nActivating this option you will have access to various tools to modify properties and sections",
    )
