...
def get_active_pose_bone(context):
    ...
    parentless_bones = [
        bone for bone in obj.data.bones if (not bone.parent or bone.parent == "")
    ]
...
class ObjectAnimRet(bpy.types.PropertyGroup):
    ...
    def _remove_source(cls, _context, old_source_armature, do_check):
        ...
        bones.remove_bones_startwith(old_source_armature, ObjectAnimRet.prefix)
    ...
    active_index: bpy.props.IntProperty(
        name="Active Index", get=_get_active_index, set=_set_active_index
    )
    old_source: bpy.props.PointerProperty(
        type=bpy.types.Object, name="Old Source Object"
    )
    source: bpy.props.PointerProperty(
        type=bpy.types.Object, name="Source Object", update=_on_update_source
    )
    ...
    active_mirror_bone: bpy.props.BoolProperty(
        name="Active Mirror (Bone)", default=False
    )
    active_mirror_constraint: bpy.props.BoolProperty(
        name="Active Mirror (Constraint)", default=False
    )

    @classmethod
    def get_offset_bone_name(cls, target_bone_name, source_bone_name):
        offset_bone_name = "{0}{1}__{2}".format(
            ObjectAnimRet.prefix, target_bone_name, source_bone_name
        )
        return offset_bone_name

    @classmethod
    def update_offset_bones(
        cls,
        target_armature,
        target_bone_name,
        source_armature,
        ...
        if bpy.context.active_object.anim_ret.is_frozen:
            return
        offset_bone_name = ObjectAnimRet.get_offset_bone_name(
        ...
        if not use_offset_bone:
            if offset_bone_name in source_armature.pose.bones:
                bones.remove_bone(source_armature, offset_bone_name)
            return

        offset_bone = bones.create_or_get_bone(source_armature, offset_bone_name)

        bones._layer(source_armature, offset_bone_name, 0, False)
        bones.set_bone_layer(source_armature, offset_bone_name, 31, True)

        offset_pbone = source_armature.pose.bones[offset_bone_name]
        offset_pbone_parent_name = (
            "" if offset_pbone.parent is None else offset_pbone.parent.name
        )
        ...
        if offset_pbone.parent is None or offset_pbone_parent_name != source_bone_name:
            print(
                "updating offset bone {0} - wrong parent: [{1}] vs [{2}]".format(
                    offset_bone_name, offset_pbone_parent_name, source_bone_name
                )
            )
            bones.set_bone_parenting(
                source_armature, offset_bone_name, source_bone_name, False
            )
            offset_bone, offset_pbone = bones.get_bone_and_pose_bone(
                source_armature, offset_bone_name
            )
            target_bone, target_pbone = bones.get_bone_and_pose_bone(
                target_armature, target_bone_name
            )

        if not bones.are_bones_same_values(
            target_armature, target_pbone, source_armature, offset_pbone
        ):
            print(
                "updating offset bone {0} - wrong transformation".format(
                    offset_bone_name
                )
            )

            matrix, head, tail, x_axis = bones.get_world_head_tail(
                target_armature, target_bone_name
            )
            bones.set_world_head_tail_xaxis(
                source_armature, offset_bone_name, head, tail, x_axis
            )
    ...
    def get_active_bone_name(self, _context):
        ...
        if _context.active_object is not None:
            if _context.active_object.type == "ARMATURE":
                return _context.active_object.data.bones[
                    self.active_index_internal
                ].name
...
class AnimRetBoneBase:
    ...
    source_bone_name: bpy.props.StringProperty(
        name="Source Bone", update=_update_source_bone_name
    )
    disabled: bpy.props.BoolProperty(
        name="Disabled", default=False, update=_update_disabled
    )
    cached: bpy.props.BoolProperty(name="Cached", default=False)
    hide_off: bpy.props.BoolProperty(
        name="Hide Off", default=True, update=_update_hide_off
    )
    influence: bpy.props.FloatProperty(
        name="Influence", default=1.0, min=0.0, max=1.0, update=_update_influence
    )
...
class ConstraintAnimRet(AnimRetBoneBase, bpy.types.PropertyGroup):
    ...
    use_constraint: bpy.props.BoolProperty(
        name="Use Constraint", update=_update_use_constraint
    )
    use_offset_bone: bpy.props.BoolProperty(
        name="Use Offset Bone", update=_update_use_offset_bone, default=True
    )
    ...
    def get_subtarget(self, anim_ret_bone):
        ...
        if self.use_offset_bone:
            result = ObjectAnimRet.get_offset_bone_name(
                target_bone_name, source_bone_name
            )
        else:
        ...
        print(
            "subtarget: {0} >> {1} >> {2}".format(
                target_bone_name, source_bone_name, result
            )
        )
    ...
    def get_name(self, _context):
        name = "{0}{1}_{2}".format(
            ObjectAnimRet.prefix, self.constraint_type, self.index
        )
    ...
    def get_constraint(self, _context):
        ...
        _, _, _, _, _, pbone_constraints, _ = AnimRetBoneBase.get_anim_ret(
            _context, self.bone_name
        )
    ...
    def mirror_constraint_data(self, _context, mirroring):
        ...
        for field in ConstraintAnimRet._flipped_mirrored_fields:
            ...
            flipped = naming.flip_side_name(val)
    ...
    def check_constraint_internal(self, _context):
        ...
        (
            ...
            pbone_constraints,
            anim_ret_constraints,
        ) = AnimRetBoneBase.get_anim_ret(_context, self.bone_name)
        ...
        ObjectAnimRet.update_offset_bones(
            obj,
            target_bone_name,
            source_armature,
            source_bone_name,
            self.use_offset_bone,
        )
    ...
    def check_mirrored_constraint(self, _context):
        if _context.active_object.anim_ret.is_frozen:
            return
        _, anim_ret, _, _, _, _, _ = AnimRetBoneBase.get_anim_ret(
            _context, self.bone_name
        )
        ...
        mirror_bone_name = naming.flip_side_name(self.bone_name)
        ...
        (
            ...
            _,
            _,
            mirror_bone_anim_ret_constraints,
        ) = AnimRetBoneBase.get_anim_ret(_context, mirror_bone_name)
    ...
    def check_constraint(self, _context):
        ...
        if self.cached or self.disabled:
            return
        _, anim_ret, _, _, anim_ret_bone, _, _ = AnimRetBoneBase.get_anim_ret(
            _context, self.bone_name
        )
...
class BoneAnimRet(AnimRetBoneBase, bpy.types.PropertyGroup):
    ...
    def check_bone_internal(self, _context):
        if _context.active_object.anim_ret.is_frozen:
            return
        _, anim_ret, _, _, _, _, anim_ret_constraints = AnimRetBoneBase.get_anim_ret(
            _context, self.bone_name
        )
    ...
    def check_mirrored_bone(self, _context):
        ...
        _, _, _, _, anim_ret_bone, _, _ = AnimRetBoneBase.get_anim_ret(
            _context, mirror_bone_name
        )
