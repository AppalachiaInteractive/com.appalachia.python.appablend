...
from appablend.retargeting.core import (BoneAnimRet, Constants,
                                        ConstraintAnimRet, ObjectAnimRet)
from appablend.retargeting.ops import *
from appablend.root_motion.core import *
...
class OBJECT_UL_targetbones(UIList):
    ...
    def draw_item(
        ...
        if self.layout_type in {"DEFAULT", "COMPACT"}:
            ...
            layout.label(
                text="",
                icon=Constants.ICON_HIDE_OFF
                if anim_ret_bone.hide_off
                else Constants.ICON_HIDE_ON,
            )
            layout.label(
                text="",
                icon=Constants.ICON_USE_MIRROR
                if anim_ret_bone.use_mirror
                else Constants.ICON_OFF,
            )
            layout.label(
                text="",
                icon=Constants.ICON_USE_OFFSET_BONE
                if use_offset_bone
                else Constants.ICON_OFF,
            )
...
class OBJECT_PT_ObjectPanel(bpy.types.Panel, PT_, UI.PROPERTIES.WINDOW.DATA):
    ...
    def do_draw(self, context, scene, layout, obj):
        ...
        row.enabled = True
        row.prop(
            anim_ret, "is_frozen", text="", toggle=True, icon=Constants.ICON_FREEZE
        )
        ...
        for bone in target_pbones:
            ...
            for con in bone.constraints:
                if con.name.startswith(ObjectAnimRet.prefix):
                    if (
                        con.target is not None
                        and con.subtarget is not None
                        and con.subtarget != ""
...
class BONE_PT_PoseBonePanel(bpy.types.Panel, PT_, UI.PROPERTIES.WINDOW.BONE):
    ...
    def do_draw(self, context, scene, layout, obj):
        ...
        BONE_PT_PoseBonePanel.draw_buttons(
            layout, source_obj, get_active_pose_bone(context)
        )
    ...
    def draw_buttons(cls, layout, source_obj, pose_bone):
        ...
        row.prop_search(
            anim_ret_bone, "source_bone_name", source_obj.data, "bones", text=""
        )

        row = layout.row()

        row.operator(
            AR_Refresh_Constraints.bl_idname, text="", icon=Constants.ICON_REFRESH
        )
        row.prop(
            anim_ret_bone,
            "use_mirror",
        ...
        for constraint in enums.constraints.ALL_:
            op = constraints.ops[constraint]
            icon = constraints.icons[constraint]
        ...
        for cd in anim_ret_constraints:
            ...
            if cd.constraint_type == "":
                ...
                icon = constraints.icons[cd.constraint_type]

            row.label(text=cd.name, icon=icon)
            icon = Constants.ICON_HIDE_OFF if cd.hide_off else Constants.ICON_HIDE_ON
            layout_state(row, state).prop(
                cd, "hide_off", text="", toggle=True, icon=icon
            )
            ...
            layout_state(row, state).prop(
                cd, "use_constraint", text="", toggle=True, icon="X"
            )
...
def register():
    ...
    bpy.types.PoseBone.anim_ret_constraints = bpy.props.CollectionProperty(
        type=ConstraintAnimRet
    )
    bpy.types.Object.anim_ret_bone_cache = bpy.props.CollectionProperty(
        type=BoneAnimRet
    )
    bpy.types.Object.anim_ret_constraints_cache = bpy.props.CollectionProperty(
        type=ConstraintAnimRet
    )
