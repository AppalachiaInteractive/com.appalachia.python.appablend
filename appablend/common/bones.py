...
import bpy
from appablend.common.core.modes import (MODE_EDIT, MODE_POSE, enter_mode_if,
                                         exit_mode_if)
from bpy.types import Bone, EditBone, Object, PoseBone
from mathutils import Matrix, Vector


def get_bone_data_path(bone_name, prop) -> str:
    path = 'pose.bones["{0}"].{1}'.format(bone_name, prop)
    return path


def get_edit_bones(obj) -> List[EditBone]:
    if hasattr(obj, "data"):
...
def is_bone_in_layer(obj, bone_name, index) -> bool:
...
def set_bone_layer(obj, bone_name, index, value) -> None:
    if not bone_name in obj.data.bones:
        return
...
def get_pose_bone(obj, bone_name) -> PoseBone:
...
def get_bone_and_pose_bone(obj, bone_name) -> Tuple[Bone, PoseBone]:
    if bone_name in obj.data.bones:
...
def set_edit_bone_matrix_by_object(
    obj, bone_name, target_object, bone_length=1.0
) -> None:
    entered, active, mode = enter_mode_if(MODE_EDIT, obj)
...
def get_pose_bone_rest_matrix_object(obj, bone_name) -> Matrix:
...
def get_pose_bone_rest_matrix_world(obj, bone_name) -> Matrix:
    entered, active, mode = enter_mode_if(MODE_POSE, obj)
...
def reset_pose_bone_location(obj, bone_name) -> None:
    entered, active, mode = enter_mode_if(MODE_POSE, obj)
...
def set_world_head_tail(obj, bone_name, head, tail) -> None:
    entered, active, mode = enter_mode_if(MODE_EDIT, obj)
...
def set_world_head_tail_xaxis(obj: Object, bone_name, head, tail, x_axis) -> None:
    entered, active, mode = enter_mode_if(MODE_EDIT, obj)
...
def are_bones_same_values(obj: Object, bone: Bone, obj2: Object, bone2: Bone) -> bool:
...
def get_edit_bone_data_dict(
    obj: Object,
) -> Dict[str, Tuple[Vector, Vector, float, bool]]:
    entered, active, mode = enter_mode_if(MODE_EDIT, obj)
...
def align_bone_x_axis(edit_bone: EditBone, new_x_axis: Vector) -> None:
...
def get_disconnected_bone_names(armature: Object) -> Set[str]:
...
def get_parent_bone_names(bone: Bone) -> Set[str]:
...
def get_child_bone_names_recursive(bone: Bone) -> Set[str]:
    bone_names = set()
