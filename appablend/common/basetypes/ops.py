...
from appablend.common.core.modes import exit_mode
from appablend.common.utils.common import print_exception
from appablend.common.utils.naming import get_logging_name
...
class OPS_(Operator):
    ...
    @classmethod
    def operator(cls, layout, icon, text):
        return layout.operator(cls.bl_idname, icon=icon, text=text)
    ...
    def poll(cls, context):
        try:
            ...
            if callable(result):
                print(
                    "[EXCP] {0}:  [do_poll]  Must correct call [{1}]".format(
