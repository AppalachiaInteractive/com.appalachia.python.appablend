...
def enter_mode(new_active_object, new_mode, unselect_current=True):
    ...
    previous_active_object_mode = (
        bpy.context.mode if not previous_active_object else previous_active_object.mode
    )
    ...
    if (
        ...
        and previous_active_object != new_active_object
        and unselect_current
    ):
...
def exit_mode(new_active_object, new_mode, unselect_current=True):
    ...
    print(
        "Exiting {0} ({1}). Returning to {2} ({3})".format(
            previous_active_object_mode,
            "NONE" if not previous_active_object else previous_active_object.name,
            new_mode,
            "NONE" if not new_active_object else new_active_object.name,
        )
    )
