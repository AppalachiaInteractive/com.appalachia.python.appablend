...
def get_files_in_dir(
    dir, prefix="", contains="", endswith="", case_sensitive=True, recursive=True
):
    ...
    for (dirpath, dirnames, filenames) in walk(dir):
        for filename in filenames:
            if prefix == "" or (
                (case_sensitive and filename.startswith(prefix))
                or (not case_sensitive and filename.lower().startswith(prefix.lower()))
            ):
                if contains == "" or (
                    (case_sensitive and contains in filename)
                    or (not case_sensitive and contains.lower() in filename.lower())
                ):
                    if endswith == "" or (
                        (case_sensitive and filename.endswith(endswith))
                        or (
                            not case_sensitive
                            and filename.lower().endswith(endswith.lower())
                        )
                    ):
