...
import bpy
import mathutils
from appablend.common.bones import get_bone_data_path
from appablend.common.core.enums import KEYFRAME
from appablend.common.utils import iters
from bpy.types import Action, FCurve, Keyframe
from mathutils import Euler, Quaternion, Vector


def insert_keyframe_extreme_by_path_and_index(
    action: Action,
    data_path: str,
    array_index: int,
    frame: int,
    ...
    return insert_keyframe_by_path_and_index(
        action,
        data_path,
        array_index,
        frame,
...
def insert_keyframe_jitter_by_path_and_index(
    action: Action,
    data_path,
    array_index,
    frame,
...
def insert_keyframe_movinghold_by_path_and_index(
    action: Action,
    data_path,
    array_index,
    frame,
    ...
    return insert_keyframe_by_path_and_index(
        ...
        needed,
        fast,
        KEYFRAME.MOVING_HOLD,
    )


def insert_keyframe_breakdown_by_path_and_index(
    action: Action,
    data_path,
    array_index,
    frame,
...
def insert_keyframe_by_path_and_index(
    action: Action,
    data_path,
    array_index,
    frame,
    ...
    needed: bool = True,
    fast: bool = False,
    keyframe_type=KEYFRAME.KEYFRAME,
):
    fcurve = action.fcurves.find(data_path, index=array_index)
    if fcurve:
        return insert_keyframe(
            fcurve, frame, value, replace, needed, fast, keyframe_type
        )


def insert_keyframe_extreme_by_path(
    action: Action,
    data_path,
    frame,
    value,
    ...
    return insert_keyframe_by_path(
        action, data_path, frame, value, replace, needed, fast, KEYFRAME.EXTREME
    )


def insert_keyframe_jitter_by_path(
    action: Action,
    data_path,
    frame,
    value,
    ...
    return insert_keyframe_by_path(
        action, data_path, frame, value, replace, needed, fast, KEYFRAME.JITTER
    )


def insert_keyframe_movinghold_by_path(
    action: Action,
    data_path,
    frame,
    value,
    ...
    return insert_keyframe_by_path(
        action, data_path, frame, value, replace, needed, fast, KEYFRAME.MOVING_HOLD
    )


def insert_keyframe_breakdown_by_path(
    action: Action,
    data_path,
    frame,
    value,
    ...
    return insert_keyframe_by_path(
        action, data_path, frame, value, replace, needed, fast, KEYFRAME.BREAKDOWN
    )


def insert_keyframe_by_path(
    action: Action,
    data_path,
    frame,
    value,
    ...
    for index in indices:
        ...
        if fcurve:
            ...
            results.append(
                insert_keyframe(fcurve, frame, v, replace, needed, fast, keyframe_type)
            )

    return results


def insert_keyframe_extreme(
    fcurve: FCurve,
    frame,
    value,
    replace: bool = False,
    ...
    return insert_keyframe(
        fcurve, frame, value, replace, needed, fast, KEYFRAME.EXTREME
    )


def insert_keyframe_jitter(
    fcurve: FCurve,
    frame,
    value,
    replace: bool = False,
...
def insert_keyframe_movinghold(
    fcurve: FCurve,
    frame,
    value,
    replace: bool = False,
    ...
    return insert_keyframe(
        fcurve, frame, value, replace, needed, fast, KEYFRAME.MOVING_HOLD
    )


def insert_keyframe_breakdown(
    fcurve: FCurve,
    frame,
    value,
    replace: bool = False,
    ...
    return insert_keyframe(
        fcurve, frame, value, replace, needed, fast, KEYFRAME.BREAKDOWN
    )
...
def insert_keyframe(
    ...
    k = fcurve.keyframe_points.insert(
        frame, value, options=options, keyframe_type=keyframe_type
    )
...
def _create_default_rotation(
    mode: str,
) -> Union[Quaternion, Euler, Tuple[Vector, float], None]:
    if mode == "rotation_quaternion":
        return mathutils.Quaternion()
    elif mode == "rotation_euler":
        return mathutils.Euler([0.0, 0.0, 0.0], "XYZ")
    elif mode == "rotation_axis_angle":
        return (Vector([0.0, 0.0, 0.0]), 0.0)


def get_rotation_at_key(
    action: Action, channel: str, frame: int
) -> Union[Quaternion, Euler, Vector]:
    options = [
            ...
            for rotation_component_index in range(rotation_part_length):
                ...
                if rotation_parts > 1:
                    if rotation_component_index > 1:
                        rotation[rotation_part_index][rotation_component_index] = v
                    else:
...
def sample_fcurve(action):
    ...
    for fcurve in action.fcurves:
        ...
        if len(points) > 0:
            print(
                "[{0}]: adding {1} points to keying data path [{2}]".format(
                    action.name, len(points), fcurve.data_path
                )
            )
            for point in points:
                insert_keyframe_breakdown(
                    fcurve, point[0], point[1], replace=False, needed=False, fast=True
                )
...
def decorate_curves(action):
    for fcurve in action.fcurves:
        ...
        for index, key in enumerate(kps):
            if index == 0 or index == len(kps) - 1:
                key.type = KEYFRAME.EXTREME
                continue

            if key.type == KEYFRAME.EXTREME:
                continue
            ...
            if round(avg, 3) == round(v, 3):
                if nv > v and v > pv:
                    kt = KEYFRAME.BREAKDOWN
                elif nv < v and v < pv:
                    kt = KEYFRAME.BREAKDOWN
                else:
                    kt = KEYFRAME.MOVING_HOLD
...
def split_action(
    master_action, new_action_name, old_clip_name, new_clip_name, start, end
):
    ...
    new_clip = UnityClipMetadata.handle_split(
        master_action, new_action, old_clip_name, new_clip_name, frame_shift
    )
    ...
    print(
        "SPLIT: [{0}] to [{1}] - [{2}-{3}] >> [{4}-{5}]".format(
            master_action.name, new_action.name, start, end, new_start, new_end
        )
    )

    new_action = copy_from_action_range(
        master_action, new_action, start, end, frame_shift
    )
...
def copy_from_action_range(master_action, new_action, start, end, frame_shift):
    ...
    for index in iters.reverse_index(new_action.fcurves):
    ...
    for curve in master_action.fcurves:
        ...
        new_curve = new_action.fcurves.new(
            curve.data_path, index=curve.array_index, action_group=group_name
        )
...
def integrate_empties_action_into_bones(obj_names, master_name):
    ...
    for action_set_key in action_sets.keys():
        ...
        for action_name in action_set:
            clean_name = (
                action_name.replace("Animation Base Layer", "")
            ...
            for master_action_name in master_action_names:
                clean_master_action_name = (
                    master_action_name.replace("Animation Base Layer", "")
                    .replace(master_name, "")
                    .strip("|")
                )
...
def update_rotations_to_quat(obj, path_check, new_data_path_prefix=""):
    ...
    actions = [
        action for action in bpy.data.actions if action.name.startswith(obj.name)
    ]
    ...
    for action in actions:
        ...
        for data_path in data_paths:
            ...
            group = next(
                (fcurve.group for fcurve in fcurves if fcurve.group is not None), None
            )
