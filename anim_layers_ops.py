...
def setup_new_layer(obj, new_track, Duplicate, blend_type):
    ...
    if action_search(obj.animation_data.action, obj.animation_data.nla_tracks) and not Duplicate:
...
class MoveAnimLayerUp(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if obj.track_list_index < len(obj.animation_data.nla_tracks)-2:
            ...
            obj.Anim_Layers[obj.track_list_index].solo, obj.Anim_Layers[obj.track_list_index+1].solo = obj.Anim_Layers[obj.track_list_index+1].solo, obj.Anim_Layers[obj.track_list_index].solo
            obj.Anim_Layers[obj.track_list_index].mute, obj.Anim_Layers[obj.track_list_index+1].mute = obj.Anim_Layers[obj.track_list_index+1].mute, obj.Anim_Layers[obj.track_list_index].mute
    ...
    def execute(self, context):
        ...
        for obj in context.selected_objects:
            for fcu in obj.animation_data.action.fcurves:
                if obj.type == 'ARMATURE' and obj.mode == 'POSE': #apply only to selected bones
...
class ResetLayerKeyframes(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        for fcu in fcurves:
            ...
            if 'scale' in fcu.data_path and anim_data.nla_tracks[obj.track_list_index].strips[0].blend_type in blend_types:
                value = 1
