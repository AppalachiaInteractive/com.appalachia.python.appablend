...
class EB_PT_menu(bpy.types.Panel, PT_, UI.VIEW_3D.UI.Tool):
    ...
    bl_icon = cspy.icons.EMPTY_ARROWS
    ...
    def do_draw(self, context, scene, layout, obj):
        ...
        col.prop(scene, 'eb_target_type') #TARGETS = ['ACTION', 'ACTION NAME', 'FILE', 'DIR']
        ...
        if scene.eb_target_type != 'DIR' and scene.eb_target_type != 'FILE':
            ...
            col.separator()
            col.operator(EB_OT_deconstruct_duplicate_bake.bl_idname, text="Deconstruct + Duplicate + Bake")
...
def register():
    bpy.types.Scene.eb_auto_bones_orientation = bpy.props.BoolProperty(name="Automatic Bones Orientation", default=True)
    bpy.types.Scene.eb_auto_target_axis = bpy.props.EnumProperty(name="Target Axis", items=(("X", "X", "X"), ("Y", "Y", "Y"), ("Z", "Z", "Z"), ("-X", "-X", "-X"), ("-Y", "-Y", "-Y"), ("-Z", "-Z", "-Z")), default = 'Z', description="The empty's target axis that the bone's X axis will match")
    bpy.types.Scene.eb_invert_x = bpy.props.BoolProperty(name="Invert X Rot", default=False)
    bpy.types.Scene.eb_invert_y = bpy.props.BoolProperty(name="Invert Y Rot", default=False)
    bpy.types.Scene.eb_invert_z = bpy.props.BoolProperty(name="Invert Z Rot", default=False)
    bpy.types.Scene.eb_current_empty_action = bpy.props.StringProperty(default="", name="Empty Action")
