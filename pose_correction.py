...
from cspy import subtypes, bones

LOCATION_CORRECTION_TYPE = [ 'LOCK', 'NEGATE', 'CLEAR' ]
LOCATION_CORRECTION_TYPE_ENUM =  cspy.utils.create_enum(LOCATION_CORRECTION_TYPE)

LOCATION_NEGATE_TYPE = [ 'EXACT', 'OFFSET', 'OBJECT', 'CANCEL' ]
LOCATION_NEGATE_TYPE_ENUM =  cspy.utils.create_enum(LOCATION_NEGATE_TYPE)

ROTATION_CORRECTION_TYPE = [ 'CLEAR' ]
ROTATION_CORRECTION_TYPE_ENUM =  cspy.utils.create_enum(ROTATION_CORRECTION_TYPE)

TRANSFORM_CORRECTION_TYPE = [ 'LOCK', 'NEGATE', 'CLEAR' ]
TRANSFORM_CORRECTION_TYPE_ENUM =  cspy.utils.create_enum(TRANSFORM_CORRECTION_TYPE)

TRANSFORM_NEGATE_TYPE = [ 'EXACT', ]
TRANSFORM_NEGATE_TYPE_ENUM =  cspy.utils.create_enum(TRANSFORM_NEGATE_TYPE)
...
class PoseCorrection(bpy.types.PropertyGroup):
    ...
    location_reference: bpy.props.FloatVectorProperty(
        name='Reference Point', subtype=subtypes.FloatVectorProperty.Subtypes.TRANSLATION)
    location_influence: bpy.props.FloatProperty(name='Influence', default=1.0,min=0.0,max=1.0)
    ...
    location_negate_co_offset: bpy.props.FloatVectorProperty(
        name='Negation Offset', subtype=subtypes.FloatVectorProperty.Subtypes.TRANSLATION)
    location_negate_co_object: bpy.props.FloatVectorProperty(
        name='Negation Point', subtype=subtypes.FloatVectorProperty.Subtypes.TRANSLATION)
    ...
    rotation_influence: bpy.props.FloatProperty(name='Influence', default=1.0,min=0.0,max=1.0)


    transform_correction_type: bpy.props.EnumProperty(
        name='Transform Correction Type', items=TRANSFORM_CORRECTION_TYPE_ENUM, default=0)
    transform_reference: bpy.props.FloatVectorProperty(
        name='Reference Point', subtype=subtypes.FloatVectorProperty.Subtypes.MATRIX, size=16)
    transform_influence: bpy.props.FloatProperty(name='Influence', default=1.0,min=0.0,max=1.0)
    ...
    def correct_pose_clear(self, context, armature, pose_bone, prop_name):        
        ...
        cspy.utils.enter_mode_simple('OBJECT')
        ...
        cspy.utils.enter_mode(armature, 'OBJECT')
        cspy.utils.enter_mode(armature, 'POSE')
    ...
    def correct_pose_location(self, context, armature, pose_bone):
        ...
        if self.location_correction_type == 'LOCK':
            other_bone_name = self.location_lock_bone_name if self.location_has_lock_bone else self.bone_name
        else:
    ...
    def correct_pose_transform(self, context, armature, pose_bone):
        ...
        if self.transform_correction_type == 'LOCK':
            other_bone_name = self.transform_lock_bone_name if self.transform_has_lock_bone else self.bone_name
        else:
