...
class _PT_RM_020_Clip_010_RootMotion(CLIP_SUBPANEL_REQ):
    ...
    def draw_curve_buttons(self, layout, armature, root_motion):
        ...
        row.operator(RM_OT_settings_to_curves.bl_idname, icon=cspy.icons.CURVE_DATA)
        row.operator(RM_OT_settings_to_curves_all.bl_idname, icon=cspy.icons.CURVE_PATH)
        row.separator()
        ...
        rows.operator(RM_OT_reset_offsets_all.bl_idname, icon = RM_OT_reset_offsets_all.bl_icon, text='')   
        o = rows.operator(RM_OT_set_offset_all.bl_idname, icon=cspy.icons.ARMATURE_DATA, text='')
        o.target, o.operation, o.phase = 'origin', 'set_to_rest', 'start'
        rows.operator(RM_OT_push_location_offsets_all.bl_idname, icon=cspy.icons.CON_LOCLIKE, text='')
        rows.operator(RM_OT_push_rotation_offsets_all.bl_idname, icon=cspy.icons.ORIENTATION_GIMBAL, text='')
        rows.operator(RM_OT_refresh_childof_constraints.bl_idname, icon=RM_OT_refresh_childof_constraints.bl_icon, text='')
    ...
    def draw_root_node_start(self, layout, armature, root_motion):
        ...
        b = layout.box()
        h = b.row(align=True)
        h.prop_search(rms, 'original_root_bone', armature, 'bones', text='Original Root', icon=cspy.icons.BONE_DATA)
        h2 = h.split()
        h2.enabled = False
        ...
        col3r = col3.row()
        o = col3r.operator(RM_OT_set_offset.bl_idname, icon=cspy.icons.POSE_HLT, text='To Rest')
        o.target, o.operation, o.phase = 'origin', 'set_to_rest', 'start'
        ...
        col3r2 = col3r.split()
        s = col3r2.operator(CURSOR_OT_set_target.bl_idname, icon=CURSOR_OT_set_target.bl_icon, text='')
        col3r2.enabled = rms.root_node is not None
        if col3r2.enabled:
            s.target_name = rms.root_node.name
        
        o = col3r.operator(RM_OT_set_offset.bl_idname, icon=cspy.icons.CURSOR, text='To Cursor')
        o.target, o.operation, o.phase = 'origin', 'set_to_cursor', 'start'
        
        col3a = col3.split()
        col3a.alert = alert
        o = col3a.operator(RM_OT_set_offset.bl_idname, icon=cspy.icons.CANCEL, text='Reset')
        o.target, o.operation, o.phase = 'origin', 'reset', 'start'

    def draw_root_motion_settings(self, layout, armature, root_motion):        
        col = layout.column()  
        r0, r1, r2, r3 = col.row(align=True), col.row(align=True), col.row(align=True), col.row(align=True)  
        col.separator()      
        ...
        r4a.prop(root_motion, 'root_motion_rot_bake_into',toggle=True,text='',icon=cspy.icons.ORIENTATION_GIMBAL)
       
        r4a.prop(root_motion, 'root_motion_rot_offset', text='Rot. Offset')  
        r4a.separator()
        o = r4a.operator(RM_OT_set_offset.bl_idname, icon=cspy.icons.CANCEL, text='Reset')
        o.target, o.operation, o.phase = 'settings', 'reset', 'start'
        r4a.separator()
        r4b.operator(RM_OT_push_location_offsets.bl_idname, icon=RM_OT_push_location_offsets.bl_icon)
        r4b.operator(RM_OT_push_rotation_offsets.bl_idname, icon=RM_OT_push_rotation_offsets.bl_icon)
    ...
    def draw_bone_offset(self, layout, armature, root_motion, bone):       
        ...
        brs = br.row()
        #brs.enabled = False
        brs.prop_search(armature.root_motion_settings, 
    ...
    def draw_bone_start_end(self, layout, armature, root_motion, bone, phase):
        ...
        o = row.operator(RM_OT_set_offset.bl_idname, icon=cspy.icons.POSE_HLT, text='Rest')
        o.target, o.operation, o.phase = bone.capitalize(), 'set_to_rest', phase.lower()
        row.separator()
        ...
        r2 = row.split()
        s = r2.operator(CURSOR_OT_set_target.bl_idname, icon=CURSOR_OT_set_target.bl_icon, text='')
        r2.enabled = getattr(rms, p0) is not None
        if r2.enabled:
            s.target_name = getattr(rms, p0).name
        
        o = row.operator(RM_OT_set_offset.bl_idname, icon=cspy.icons.CURSOR, text='Cursor')
        o.target, o.operation, o.phase = bone.capitalize(), 'set_to_cursor', phase.lower()

        row.separator()
        r2 = row.split()
        r2.alert = alert
        o = r2.operator(RM_OT_set_offset.bl_idname, icon=cspy.icons.CANCEL, text='Reset')
        o.target, o.operation, o.phase = bone.capitalize(), 'reset', phase.lower()
