...
class UnityKeyframeData:
    ...
    def process_key_row(cls, row, key_offset):
        obj_name,clip_name,data_path,array_index,frame,value = cls.parse_key_row(row, key_offset)
    ...
    def parse_key_files(cls, context, dir_path, key_offset):
        filepaths = cspy.files.get_files_in_dir(dir_path, '','','.keys.txt')
            ...
            for clip_name in obj_key_datas.keys():
                ...
                for data_path in clip_key_datas.keys():
                    ...
                    for array_index in data_path_key_datas.keys():       
                        ...
                        if current_only and obj_name != context.active_object.animation_data.action.name:
                            continue
                        ...
                        for keyframe in keyframes:
                            cspy.actions.insert_keyframe(fcurve, keyframe.frame, keyframe.value, needed=True, fast=True)
